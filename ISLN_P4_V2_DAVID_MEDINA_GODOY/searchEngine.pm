...
sub search_word{
	...
	if(not exists $hash_ref->{$title}){
	    $hash_ref->{$title}{"content"} = $content;
            $hash_ref->{$title}{"title"} = $title;	    
	    $hash_ref->{$title}{"search_elem"} = "";
	    $hash_ref->{$title}{"partial_content"} = "";
	    $hash_ref->{$title}{"word_count"} = scalar split(/\s+/, $title);
            $hash_ref->{$title}{"word_count"} += scalar split(/\s+/, $content);
	...
	if("$title $content" =~ m/\b(${word})\b/i){
	    ...
	    #Contamos num de apariciones de la palabra en la noticia
	    #$hash_ref->{$title}{$word_key} = 0;
	    $title = $hash_ref->{$title}{"title"};
	    $content = $hash_ref->{$title}{"content"};
	    #$hash_ref->{$title}{$word_key};
	    #print $link;<STDIN>;
	    $hash_ref->{$title}{"word_key"}{$word_key}{"count"}=0;
	    while($title =~ m/\b${word_key}\b/gi){
		$hash_ref->{$title}{"word_key"}{$word_key}{"count"}++;
	    }
	    while($content =~ m/\b${word_key}\b/gi){
		$hash_ref->{$title}{"word_key"}{$word_key}{"count"}++;
	    }
	    $hash_ref->{$title}{"word_key"}{$word_key}{"tf"} = 
		$hash_ref->{$title}{"word_key"}{$word_key}{"count"}/($hash_ref->{$title}{"word_count"} * 1.0);
	    #;print "\n";<STDIN>;
...
#    say $elems_refs[0]->{"title"};
    ...
    return ($hash_ref,$num_docs,$num_docs_found);
}

sub search{
    my $search_content = shift;
    my $url = shift;
    my $one_word_mode = shift;
    my $hash_ref=();
    ...
    foreach(@words){
	#print $_."\n";
	($hash_ref,$num_docs,$num_docs_found)=search_word($_,$url,$hash_ref);
	#$NI{$_} = $num_docs_found;
	$IDF{lc $_} = log($num_docs/$num_docs_found);
	#print $IDF{$_};print"$_<--\n";
    }
    ...
    foreach $doc(keys %$hash_ref){
	$RELEVANCE{$doc} = 0;
	foreach $word(keys $hash_ref->{$doc}{"word_key"}){
	    #$doc = $hash_ref->{$doc_title}{"title"};
	    #$word_key; = $hash_ref->{$doc_title}{"word_key"}{$word_key};
	    #print $doc_title;<STDIN>;
	    $TF{$word}{$doc} = $hash_ref->{$doc}{"word_key"}{$word}{"tf"};
	    #print $TF{$word}{$doc};<STDIN>;
	    #print $word;print "---\n";
	    #print $IDF{$word};<STDIN>;
	    
	    #if(defined $hash_ref->{$doc}){
	    
		$TF_IDF{$word}{$doc} = $TF{$word}{$doc} * $IDF{$word}; 
		$RELEVANCE{$doc}+= $TF_IDF{$word}{$doc};
	    #}
	}
    
	#print $hash_ref->{$doc}{"search_elem"};
    }
    foreach (sort { ($RELEVANCE{$b} cmp $RELEVANCE{$a}) } keys %RELEVANCE) 
    {
    #print "$_: $RELEVANCE{$_}\n";
	print "<div title=\"tdf-idf: $RELEVANCE{$_}\">";
	print $hash_ref->{$_}{"search_elem"};
	print "</div>";
    }
    #<STDIN>;
...
sub idf{
    ...
    my $n = shift;
    my $relems = shift;
    my $num_docs = scalar keys %$relems;
    foreach(keys %$relems){
